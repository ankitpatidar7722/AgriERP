...
export function useIntelligentColumnSizing<TData>(
  ...
  const analyzeContent = useCallback((columnId: string, sampleData: TData[]) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- dynamic column access on a row of unknown shape
  ...
  const columnAnalysis = useMemo(() => {
    ...
    columns.forEach(column => {
      ...
      const columnMeta = column.meta as { minWidth?: number; maxWidth?: number } | undefined
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- tanstack table instance
  const autoResizeAllColumns = useCallback((table: any) => {
    if (!table?.getAllColumns) return

    const allColumns = table.getAllColumns()
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- tanstack column instance
