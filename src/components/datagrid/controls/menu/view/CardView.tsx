...
export function CardView<TData>({
  ...
  // Render a column's value for the card title/subtitle. Prefer the column's own
  // cell renderer (badges, name+subtitle, etc.) so columns that use accessorFn+id
  // with no accessorKey (e.g. this app's DataTable wrapper) still show real content
  // instead of "—"; fall back to the plain value for bare accessorKey columns.
  const renderCardValue = useCallback((column: ColumnDef<TData>, item: TData) => {
    const key = ((column as any).accessorKey as string) || (column.id as string)
    const raw = key ? (item as any)[key] : undefined
                  ...
                  `}>
                    ...
                    <div className="pl-3 pr-2.5 py-2.5">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {/* Title + subtitle — render each column's own cell so rich
                            columns (name+subtitle, badges) and accessorFn+id columns
                            show real content instead of "—". */}
                        <div className="flex-1 min-w-0 flex items-baseline gap-1.5">
                          <div className={`${cfg.titleText} font-semibold text-[rgb(var(--fg-default))] truncate leading-tight min-w-0`}>
                            {displayColumns.length > 0 && renderCardValue(displayColumns[0], item)}
                          </div>
                          {displayColumns.length > 1 && (
                            <div className={`${cfg.subtitleText} dg-card-subtitle font-medium truncate min-w-0`}>
                              {renderCardValue(displayColumns[1], item)}
                            </div>
                          )}
                        </div>
